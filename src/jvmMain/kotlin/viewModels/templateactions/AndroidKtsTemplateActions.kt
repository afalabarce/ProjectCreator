...
class AndroidKtsTemplateActions: TemplateProjectAction() {
    private fun getModuleNames(projectPath: Path): List<String> {
        val settingsGradle = projectPath.toFile().listFiles { f -> f.name.lowercase().startsWith("settings.gradle.kts") }?.firstOrNull()
        if (settingsGradle != null) {
            val settingsLines = settingsGradle.readLines(Charsets.UTF_8)
            val regexInclude = ".*?include\\s*\\(\\s*(.*?\\s*)\\)\\s*.*?".toRegex(setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL, RegexOption.MULTILINE))
            return regexInclude.findAll(settingsLines.joinToString(" ")).flatMap {matchResult ->
    ...
    private fun updateAndroidProjectName(projectName: String, projectPath: Path): Boolean {
        try {
            val settingsGradle =
                projectPath.toFile().listFiles { f -> f.name.lowercase().startsWith("settings.gradle.kts") }?.firstOrNull()
            if (settingsGradle != null) {
    ...
    override fun loadProject(templateProject: TemplateProject, onLoadTemplateProject: (TemplateProject) -> Unit) {
        ...
        val settingsFile = templatePath.listFiles()?.firstOrNull { x -> x.name == "settings.gradle.kts" }
        val buildFile = templatePath.listFiles()?.firstOrNull { x -> x.name == "build.gradle.kts" }
        val buildVersionPath = File("${templateProject.path}${File.separator}buildSrc${File.separator}src${File.separator}main${File.separator}kotlin${File.separator}")
        val buildVersion = buildVersionPath.listFiles()?.firstOrNull { x -> x.name == "BuildVersion.kt" }

        if (settingsFile != null && buildVersion != null) {
            val settingsContent = settingsFile.readLines(charset = Charsets.UTF_8)
            val buildContent = buildVersion.readLines(charset = Charsets.UTF_8)
            ...
            val regexAppId = ".*?applicationId\\s*=\\s*\"(.*?)\"".toRegex(setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL))
            val packageMatch = regexAppId.find(buildContent.firstOrNull { x -> x.contains("applicationId") } ?: "")
            val packageName = if (packageMatch != null) packageMatch.groups[1]?.value ?: "" else ""
            ...
            try {
                ...
                if (templateProject.androidSettings.originalPackageName == templateProject.androidSettings.packageName) {
                    ...
                    this.launch(Dispatchers.IO) {
                        ...
                        if (this@AndroidKtsTemplateActions.copyFolder(Path(templateProject.path), Path(folderProject))) {
                            this@AndroidKtsTemplateActions.renameProject(
